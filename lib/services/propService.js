...
export async function taxonomyEntriesPath(content) {
  console.log("content", content);
  const contentsHandler = await TAXONOMYAPI.getTaxonomies(content);
  const contents = dataFormatter.deserialize(contentsHandler);
  let allData = contents;
  ...
  while (current_page < last_page) {
    current_page = current_page + 1;
    const contentsHandler = await TAXONOMYAPI.getTaxonomies(
      content,
      `?page[number]=${current_page}`
    );
    const contents = dataFormatter.deserialize(contentsHandler);

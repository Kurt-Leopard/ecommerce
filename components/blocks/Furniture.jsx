...
export default function Banner({ block, mediaHandler }) {
  ...
  const fetchdata = useCallback(async () => {
    ...
    try {
      const res = await BaseApi.get(
        ...
        }/api/contents/items/entries?page[size]=8&page[number]=${currentPage}&includes=blueprintData,mediaHandler&filter[taxonomies][product-category]=${
          furniture || ""
        }&filter[sites.id]=${process.env.NEXT_PUBLIC_MICROSITE_ID}`
      );
      setTotalItems(Math.ceil(parseInt(res?.data?.meta?.total) / 8));
      setLastPage(parseInt(res?.data?.meta?.last_page));
  ...
  const truncateDescription = (description, minLength = 80) => {
    if (description && description.length > minLength) {
      return description.slice(0, minLength) + "...";
    }
    return description;
  };
      ...
      {isLoading ? (
        ...
        <div className="flex flex-wrap gap-3 items-center my-5">
          {products?.map((item, index) => {
            const description = item?.attributes?.data?.main?.description;
            const truncatedDescription = truncateDescription(description);

            return (
              <Link href={item?.attributes.route_url} key={index}>
                <div
                  key={index}
                  className={`group ${totalItems % 8 > 2 ? "mx-auto" : ""}`}
                >
                  <Image
                  ...
                  />
                  <div className="bg-[#0C4A6E] group-hover:bg-white w-[400px] mx-auto my-5 rounded-xl border shadow-lg py-4">
                    ...
                    <div className="flex justify-between items-center px-4 py-2 text-[#D3D3D3]">
                      ...
                      <svg
                        ...
                        strokeWidth={1.5}
                        stroke="currentColor"
                        className="w-12 h-12 text-[#D3D3D3] group-hover:text-[#0C4A6E]"
                      >
                        <path
                          strokeLinecap="round"
